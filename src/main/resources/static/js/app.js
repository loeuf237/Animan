...
async function playEpisode(btn) {
  ...
  try {
    ...
    // 1. Chercher si la tâche locale est complétée
    try {
      const statusResp = await fetch('/api/download/status');
      const tasksList = await statusResp.json();
      const localTask = tasksList.find(t => 
        t.animeName.toLowerCase() === animeName.toLowerCase() && 
      ...
      if (localTask) {
        ...
        showToast('Lecture depuis le stockage local (Ultra Fluide) 🚀', 'success');
        try {
          const subsResp = await fetch(`/api/media/subtitles/${localTask.id}`);
          const subsList = await subsResp.json();
          tracks = subsList.map(s => ({
            kind: 'captions',
            label: s.label,
            srclang: s.srclang,
            src: s.src,
            default: s.srclang === 'fr'
          }));
        } catch (e) {
          console.warn('Erreur chargement sous-titres locaux:', e);
        }
      }
    } catch(e) {
      console.warn("Impossible de vérifier le statut local, fallback remote:", e);
    }

    // 2. Si non trouvé localement, résoudre le flux à distance
    if (!isLocal) {
      let subtitleUrl = '';
      if (source === 'voir-anime') {
    ...
    // Vider et reconstruire les sources et tracks dans le DOM pour forcer le rechargement propre
    vidEl.innerHTML = `<source id="player-source" src="${videoUrl}" type="video/mp4"/>`;
    tracks.forEach(t => {
      ...
      trackEl.src = t.src;
      if (t.default) trackEl.setAttribute('default', '');
    ...
    // 3. Auto-marquage comme vu / watched dans la base et mise à jour dynamique de l'UI
    if (animeUrl) {
      try {
        ...
        // Mise à jour de l'UI en direct sans recharger
        const epRow = btn.closest('.episode-item');
        if (epRow) {
          epRow.classList.add('ep-watched');
          if (!epRow.querySelector('.ep-watched-badge')) {
            const sizeSpan = epRow.querySelector('.episode-size');
            if (sizeSpan) {
              const watchedBadge = document.createElement('span');
              watchedBadge.className = 'ep-watched-badge';
              watchedBadge.title = 'Déjà vu';
              watchedBadge.textContent = '👁️ Vu';
              
              const epDone = epRow.querySelector('.ep-done-badge');
              if (epDone) {
                epDone.after(watchedBadge);
              } else {
                sizeSpan.before(watchedBadge);
              }
            }
          }
        }
      } catch (err) {
        console.warn("Erreur auto-marquage vu:", err);
      }
    }
...
function renderTaskActions(task) {
  ...
  if (task.status === 'PENDING') {
    priorityBtns = `
      <button class="btn btn-sm" style="background: rgba(124,106,247,0.12); border: 1px solid rgba(124,106,247,0.25); color: var(--accent);" onclick="moveTaskToTop('${id}')">🔝 Priorité</button>
      ${animeName ? `<button class="btn btn-sm" style="background: rgba(124,106,247,0.12); border: 1px solid rgba(124,106,247,0.25); color: var(--accent);" onclick="moveSeriesToTop(\`${animeName.replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`)">🎬 Priorité Série</button>` : ''}
    `;
  }
  ...
  if (task.status === 'PENDING' || task.status === 'SCHEDULED') {
    return priorityBtns + `<button class="btn btn-sm btn-danger" onclick="cancelTask('${id}')">✕ Annuler</button>`;
  }
  return '';
}
...
async function moveTaskToTop(id) {
  try {
    ...
    const data = await resp.json();
    if (data.success) {
      showToast('Priorité de la tâche mise à jour 🔝', 'success');
...
async function moveSeriesToTop(animeName) {
  try {
    ...
    const data = await resp.json();
    if (data.success) {
      showToast('Priorité de la série mise à jour 🎬', 'success');
      setTimeout(() => window.location.reload(), 800);
    } else {
      showToast('Erreur: ' + data.error, 'error');
    }
